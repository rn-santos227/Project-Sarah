...
import com.project.Doeville.entities.triggers.doors.PharmacyDoor;
...
public class Pharmacy extends StaticEntity {
	private DisplayDialog dd;
	private HUD hud;
	private PharmacyDoor door;
	private Transition tran;
	private float x, y;
	
	public Pharmacy(Handler handler, HUD hud, DisplayDialog dd, Transition tran, float x, float y) {
		...
		door = new PharmacyDoor(handler, hud, dd, 588, 928);
	}
	...
	public void render(Graphics g) {
		g.drawImage(Assets.Pharmacy, (int)(x - handler.getGameCamera().getxOffset()),(int)(y - handler.getGameCamera().getyOffset()), width, height, null);
	...
	public void examine() {
		if(door.getCollisionBounds(0f, 0f).intersects(Player.examineBox)) door.examine();
	...
	public Entity clone() {
		Entity e = new Pharmacy(handler, hud, dd, tran, x, y);
		return e;
	}
}
