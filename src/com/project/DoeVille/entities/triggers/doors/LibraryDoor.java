...
public class LibraryDoor extends Entity {
	private HUD hud;
	private DisplayDialog dd;
	private float x, y;
	public LibraryDoor(Handler handler, HUD hud, DisplayDialog dd, float x, float y) {
	...
	public void examine() {
		if(dd.getaBox().getTimer() == 0) {
			if(Player.onHand.equals("ID Card")) {
				...
				Player.messages = "You will need an ID Card to enter the library.";
			}
		} else {
	...
	public Entity clone() {
		Entity e = new LibraryDoor(handler, hud, dd, x, y);
		return e;
	}
}
