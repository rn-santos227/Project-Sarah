...
public class ITWorkplaceDoor extends Entity {
	private HUD hud;
	private DisplayDialog dd;
	private float x, y;
	public ITWorkplaceDoor(Handler handler, HUD hud, DisplayDialog dd, float x, float y) {
		...
		bounds.x = 0;
		bounds.y = 0;
		bounds.width = (Tile.t_Width/2) + 11;
	...
	public void examine() {
		if(dd.getaBox().getTimer() == 0) {
			if(Player.onHand.equals("ID Card")) {
				Player.running = false;
			} else {
				SoundEffect.Cancel.play();
				Player.dID = DialogID.information;
				Player.messages = "You will need an ID Card to enter the company.";
			}
		} else {
	...
	public Entity clone() {
		Entity e = new ITWorkplaceDoor(handler, hud, dd, x, y);
		return e;
	}
}
